"""
Main CLI entry point for TikTok Analyzer Pro.
Orchestrates video scraping, transcription, and analysis.
"""
...
logger = get_logger("analyze_tiktok")

DEFAULT_TRANSCRIPTS_PATH = config.TRANSCRIPTS_FILE
...
def summarize_transcripts(transcripts_file: str, output_file: str) -> None:
    ...
    with open(output_file, "w", encoding="utf-8") as out:
        ...
        out.write(
            "This document highlights the specific recommendations, protocols, and advice discussed in the videos.\n\n"
        )
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Download, transcribe, and summarize a TikTok profile."
    )
    parser.add_argument(
        "target",
        help="The TikTok profile URL (e.g., https://www.tiktok.com/@username) or just the @username",
    )
    parser.add_argument(
        "--limit",
        type=int,
        default=50,
        help="Maximum number of videos to download and transcribe (default: 50, use 0 for all)",
    )
    parser.add_argument(
        "--transcripts-path",
        default=None,
        help="Path to shared transcripts cache file (default: ./transcripts.md)",
    )
    parser.add_argument(
        "--api-key",
        default=None,
    ...
    cache = load_transcript_cache(str(transcripts_path))
    if os.path.exists(transcripts_file):
        cache.update(load_transcript_cache(transcripts_file))

    logger.info(f"Fetching videos from profile...")
    entries = get_video_entries(profile_url)

    if not entries:
        logger.error("No videos found. Check the profile URL.")
        sys.exit(1)
    ...
    logger.info(f"Found {len(entries)} videos. Loading Whisper model...")

    model, device = load_whisper_model(config.WHISPER_MODEL)
    if device == "cuda":
    ...
    for idx, entry in enumerate(entries):
        ...
        try:
            ...
            append_to_transcripts_file(str(transcripts_path), title, video_url, transcript)
            if video_id:
                cache[video_id] = transcript
        except Exception as e:
            logger.error(f"Error on video {idx+1}: {e}")
        finally:
