...
def classify_video(transcript):
    ...
    if len(transcript) < 200:
        return None
    if any(x in t for x in ['peptide', 'bpc', 'tb500', 'ghk', 'ss31', 'mots-c', 'sermerall', 'epitalon', 'foxo', 'selank', 'semax', 'kpv', 'dsip', 'melanotan']):
        if any(x in t for x in ['stack', 'protocol', 'phase', 'experiment']):
            return 'peptide_protocol'
        return 'peptide_info'
    if any(x in t for x in ['retatrutide', 'tirzepatide', 'semaglutide', 'glp']):
        return 'glp1_fat_loss'
    if any(x in t for x in ['testosterone', 'trt', 'hormones', 'estrogen']):
        return 'hormones'
